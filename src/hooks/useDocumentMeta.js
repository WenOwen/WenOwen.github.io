...
import { identity, seo } from '../content/site.config.js';
 ...
 * 
 * Every title/description/path comes from `seo` in site.config.js —
 * this file holds no copy of its own.
 */
...
export function useDocumentMeta() {
    ...
    useEffect(() => {
        ...
        if (ogUrl) ogUrl.setAttribute('content', `${identity.siteUrl}${meta.path}`);

        // Update canonical link to ensure virtual routes are correctly indexable as separate pages
        const canonicalTag = document.querySelector('link[rel="canonical"]');
        if (canonicalTag) {
            canonicalTag.setAttribute('href', `${identity.siteUrl}${meta.path}`);
        }
