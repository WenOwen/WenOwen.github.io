...
export const AudioProvider = ({ children }) => {
    ...
    const play = useCallback((soundName, { loop = false, volume = 1.0 } = {}) => {
        ...
        const soundPaths = {
            'szumwiatru': '/sounds/szumwiatru.wav', // Szum wiatru w pokoju About
            'szummiasta': '/sounds/szummiasta.wav', // Szum miasta w pokoju The Gallery
            'uchyleniedrzwi': '/sounds/uchyleniedrzwi.wav', // Skrzypienie przy najechaniu
            'otwarciedrzwi': '/sounds/otwarciedrzwi.wav',   // Otwarcie głównych/bocznych drzwi
            'zamknieciedrzwi': '/sounds/zamknieciedrzwi.wav' // Zamykanie drzwi
        };
        const path = soundPaths[soundName] || `/sounds/${soundName}.wav`;
        ...
        if (playPromise !== undefined) {
            playPromise.catch(error => {
                if (error.name === 'NotAllowedError') {
                    ...
                    // Silently fail if a file is missing, to keep the console clean
                } else {
