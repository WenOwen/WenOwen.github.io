...
// Derived from `identity.siteUrl` in the site config (the bare host plus its
// `www.` variant), with localhost / 127.0.0.1 so local development still works.
const SITE_HOST = (() => {
    try {
        return new URL(identity.siteUrl).hostname.replace(/^www\./, '');
    } catch {
        // Tolerate a protocol-less host in the config (e.g. "wu5.dev")
        return String(identity.siteUrl || '')
            .replace(/^https?:\/\//, '')
            .split('/')[0]
            .replace(/^www\./, '');
    }
...
const ALLOWED_ORIGINS = [
    ...(SITE_HOST ? [SITE_HOST, `www.${SITE_HOST}`] : []),
    'localhost',
    '127.0.0.1',
];
