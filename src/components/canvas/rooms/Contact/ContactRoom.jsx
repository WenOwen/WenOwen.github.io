...
import { socials, identity } from '../../../../content/site.config.js';
...
const ContactRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    const handleMailSelect = () => {
        // Awaryjne przekierowanie mailto: (adres pochodzi z site.config.js)
    ...
    // Signposts floating in the water: the 3D slots below are fixed (they were
    // hand-placed), but every label and link comes from `socials` in the site
    // config. The non-message socials fill the four outer slots and the
    // `message` social keeps the middle slot that triggers the mailto:.
    const socialBarrels = useMemo(() => {
        const slots = [
            { position: isMobile ? [-1.2, 0.5, -10] : [-3, 0.5, -10], rotation: [0, 0.2, 0] },
            { position: isMobile ? [-1.5, -0.3, -7] : [-5, -0.3, -8], rotation: [0, 0.3, 0] },
            { position: isMobile ? [1.2, 0.5, -10] : [3, 0.5, -10], rotation: [0, -0.2, 0] },
            { position: isMobile ? [1.5, -0.3, -7] : [5, -0.3, -8], rotation: [0, -0.3, 0] },
            { position: isMobile ? [0, -0.7, -6] : [0, -0.7, -7], rotation: [0, 0, 0] },
        ];
        const message = socials.find(s => s.id === 'message');
        const rest = socials.filter(s => s.id !== 'message');
        return [...rest, ...(message ? [message] : [])]
            .slice(0, slots.length)
            .map((social, i) => ({ ...social, ...slots[i] }));
    }, [isMobile]);
    ...
    return (
        <group ref={groupRef} position={[0, -0.7, -5]}>
            ...
            {/* 🛢️ SOCIAL BARRELS (Floating in water) — labels & links come from the config */}
            {socialBarrels.map((social) => (
                <SocialBarrel
                    key={social.id}
