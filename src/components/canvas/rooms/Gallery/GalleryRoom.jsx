...
import { useGalleryProjects } from '../../../../hooks/useContent';
import { projects as siteProjects, techLogos } from '../../../../content/site.config.js';
...
// Both are derived from `projects` in src/content/site.config.js:
// `art` becomes the card artwork and `tech` the logo badges on the back.
// The art generator only writes a badge for the stems listed in `techLogos`
// (`<stem>logo.webp`), so stems without artwork are dropped here — useTexture()
// throws on a missing file, which would take the whole room down with it.
const TECH_LOGO_FILES = new Set(techLogos.map((t) => t.file));

const FALLBACK_PROJECTS = siteProjects.map((p) => ({
    id: p.id,
    title: p.title,
    front: `/textures/gallery/${p.art}przod.webp`,
    painted: `/textures/gallery/${p.art}przod_painted.webp`,
    url: p.url,
    description: p.description,
    techStack: p.tech
        .map((tech) => `${tech}logo`)
        .filter((file) => TECH_LOGO_FILES.has(file))
        .map((file) => `/textures/gallery/${file}.webp`)
}));
...
const GalleryRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    // Content comes from site.config.js (FALLBACK_* stays as a guard)
    const configProjects = useGalleryProjects();
    const activeProjects = configProjects || FALLBACK_PROJECTS;
    ...
    // (the badge files come from `techLogos` in the site config)
    const allLogos = useMemo(() => {
        return techLogos.map((t) => (
            canHover
                ? `/textures/gallery/${t.file}_painted.webp`
                : `/textures/gallery/${t.file}.webp`
        ));
    }, [canHover]);
