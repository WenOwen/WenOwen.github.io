...
import { identity, projects, awards as siteAwards } from '../../../../content/site.config.js';
...
const IntroMilestone = ({ z, scrollProgressRef }) => {
    ...
    return (
        <group ref={groupRef} position={[0, 0, z]}>
            ...
            {/* Subtitle. Shows the handle, or the role line when the handle just
                repeats the name (name "wu5" + handle "WU5" reads as noise).
                Always rendered so brandRef below stays valid for the anims. */}
            <Text
                ref={brandRef}
                position={[0, 4.3, 0.1]}
                fontSize={0.45}
            ...
            >
                {identity.handle && identity.handle.toLowerCase() !== identity.name.toLowerCase()
                    ? `(${identity.handle})`
                    : identity.aboutSubtitle}
            </Text>
...
/**
 * Local fallback for the awards overlay.
 *
 * The live data comes from `useAwards()` (now backed by site.config.js). This
 * mirrors its shape so the room still renders if that hook ever returns nothing.
 * Content is factual open-source milestones — this site displays no invented
 * awards; see `awards` in site.config.js.
 */
const toPlaque = (a) => ({
    label: a.title,
    date: new Date(a.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
    image: `/textures/about/${a.file}.webp`,
    url: a.url || null,
});

/** Path to a plaque's art. Falls back to the first award if the slot is empty. */
const plaqueArt = (a, painted) => {
    const entry = a || siteAwards[0];
    return `/textures/about/${entry.file}${painted ? '_painted' : ''}.webp`;
};
...
const AWARDS_DATA = {
    featured: {
        ...
        items: projects.slice(0, 3).map((p) => ({
            label: p.title,
            date: p.stars > 0 ? `${p.stars} stars` : 'open source',
            image: `/textures/gallery/${p.art}przod.webp`,
            url: p.url,
        })),
        platformConfig: {
            label: 'HONOR',
    ...
    sotd: {
        ...
        items: siteAwards.slice(0, 4).map(toPlaque),
        platformConfig: {
            label: 'RELEASE',
            color: '#1a1a1a',
            icon: '🏆'
    ...
    sotm: {
        ...
        layout: 'certificate_grid',
        title: 'Milestones',
        items: siteAwards.slice(4).map(toPlaque),
        platformConfig: {
            label: 'MILESTONE',
            color: '#1a1a1a',
            icon: '📅'
...
const AwardsMilestone = ({ z, scrollProgressRef }) => {
    // Content comes from site.config.js (fallback keeps the room renderable)
    const configAwards = useAwards();
    const awardsData = configAwards || AWARDS_DATA;
    ...
    // Load textures. The three large plaques take their art from the first three
    // entries of `awards` in site.config.js (order: SOTD-slot, SOTM-slot, SOTY-slot).
    const sotyTexture = useLoader(THREE.TextureLoader, plaqueArt(siteAwards[2], false));
    const sotdTexture = useLoader(THREE.TextureLoader, plaqueArt(siteAwards[0], false));
    const sotmTexture = useLoader(THREE.TextureLoader, plaqueArt(siteAwards[1], false));
    const sotyPaintedTexture = useLoader(THREE.TextureLoader, isTouch ? plaqueArt(siteAwards[2], false) : plaqueArt(siteAwards[2], true));
    const sotdPaintedTexture = useLoader(THREE.TextureLoader, isTouch ? plaqueArt(siteAwards[0], false) : plaqueArt(siteAwards[0], true));
    const sotmPaintedTexture = useLoader(THREE.TextureLoader, isTouch ? plaqueArt(siteAwards[1], false) : plaqueArt(siteAwards[1], true));
