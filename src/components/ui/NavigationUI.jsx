...
import { useLocale, useT } from '../../i18n/locale.jsx';
import { S, ROOM_NAMES } from '../../i18n/strings.js';
...
// `label` comes from i18n/strings.js so the map follows the active locale.
const ROOMS = [
    { id: 'about', x: 43, y: 38 },      // Paper airplane (left side)
    { id: 'gallery', x: 43, y: 72 },    // City buildings (bottom left)
    { id: 'contact', x: 57, y: 25 },    // Pier/dock (top right)
    { id: 'studio', x: 57, y: 55 },     // Monitors stack (right side)
];
...
const NavigationUI = () => {
    ...
    const t = useT();
    const roomName = (id) => t(ROOM_NAMES[id]);
    ...
    return (
        <div className="navigation-ui">
            ...
            {/* Right side controls. The container is always mounted so the
                language toggle is reachable before entering the corridor; the
                room controls only appear once you are inside. */}
            <div className={`nav-controls ${isMenuOpen || isAudioMenuOpen ? 'menu-open' : ''} ${isUIHidden ? 'ui-hidden' : ''}`}>
                ...
                <button
                    ...
                    aria-label={t(S.switchToZh)}
                    title={t(S.language)}
                >
                    <span className="lang-label">{t(S.langShort)}</span>
                </button>
                {hasEntered && (
                    <>
